...
class App extends Component {
  ...
  menuRenerer=()=>{
    ...
    return this.menuRegist.map((item,index)=>{
      let isOn = false;
      if(selectMenu === item.key || (selectMenu==='readDetail' && item.key==='read')){
        isOn = true;
      }
      return  <div className={cn('menu-sell-item',{on:isOn})} data={item.key} onClick={this.onClickSelectMenu} key={index}>
      {item.label}
    </div>
    })
  }
  ...
  getTextList=async()=>{
    let result = []
    const resList = await IconSDK.iconService.call(
      IconSDK.callBuild({
  ...
  contentRenderer=()=>{
    ...
    switch (selectMenu) {
      ...
      case 'read':
        return <IconNovelRead readDetailSelectMenu={this.readDetailSelectMenu} {...this.props} address={address} getTextList={this.getTextList} {...this.state} />
      case 'write':
        return <IconNovelWrite {...this.props} address={address} getTextList={this.getTextList}/>
      case 'readDetail':
        return <IconNovelReadDetail selectItem={selectItem} {...this.props} address={address} getTextList={this.getTextList} {...this.state} connectAddress={this.connectAddress}/>
      default:
        break;
    }
  }
    ...
    return (
      <AppWrraper>
          <div className="gnb-title">
              ...
              <div className="login" onClick={this.onClickLogin}>{address!==''?address:'Login'}</div>
            </div>
